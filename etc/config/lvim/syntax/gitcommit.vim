...
syn region  gitcommitAuthor	matchgroup=gitCommitHeader start=/\%(^. \)\@<=\%(作者\|提交者\|日期\):/ end=/$/ keepend oneline contained containedin=gitcommitComment transparent
...
syn match   gitcommitNoChanges	"\%(^. \)\@<=没有修改$" contained containedin=gitcommitComment
...
syn region  gitcommitUntracked	start=/^\z(.\) 未跟踪的文件:$/ end=/^\z1\=$\|^\z1\@!/ contains=gitcommitHeader containedin=gitcommitComment containedin=gitcommitComment contained transparent fold
...
syn region  gitcommitDiscarded	start=/^\z(.\) 尚未暂存以备提交的变更：$/ end=/^\z1\=$\|^\z1\@!/ contains=gitcommitHeader,gitcommitDiscardedType containedin=gitcommitComment containedin=gitcommitComment contained transparent fold
...
syn region  gitcommitSelected	start=/^\z(.\) 要提交的变更：$/ end=/^\z1$\|^\z1\@!/ contains=gitcommitHeader,gitcommitSelectedType containedin=gitcommitComment containedin=gitcommitComment contained transparent fold
...
syn region  gitcommitUnmerged	start=/^\z(.\) 未合并的路径：$$/ end=/^\z1\=$\|^\z1\@!/ contains=gitcommitHeader,gitcommitUnmergedType containedin=gitcommitComment containedin=gitcommitComment contained transparent fold
